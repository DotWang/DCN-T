...
def get_abs_indices(init_label_map, num_spixels_width):
    ...
    #print(init_label_map.shape, relative_spix_indices.shape)
    #print(type(init_label_map[0]),type(relative_spix_indices[0]))
    abs_spix_indices = (init_label_map[:, None].float() + relative_spix_indices[None, :, None].float()).reshape(-1).long()
...
def get_hard_abs_labels(affinity_matrix, init_label_map, num_spixels_width):
    ...
    relative_spix_indices = torch.cat([r - num_spixels_width, r, r + num_spixels_width], 0)
    label = init_label_map.float() + relative_spix_indices[relative_label].float()
    return label.long()
