...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self,
        ...
        last_inp_channels = pre_stage_channels

        #last_inp_channels.append(64)

        self.channels = last_inp_channels

        # self.conv_1_ac = Conv1x1BNReLU(self.channels[0] + stage1_out_channel, self.channels[0])
        # self.conv_1_ace = Conv1x1BNReLU(int(2 * self.channels[0]) + stage1_out_channel, self.channels[0])
        # self.conv_1_aceg = Conv1x1BNReLU(int(3 * self.channels[0]) + stage1_out_channel, self.channels[0])
        #
        # self.conv_2_ce = Conv1x1BNReLU(int(2 * self.channels[1]), self.channels[1])
        # self.conv_2_ceg = Conv1x1BNReLU(int(3 * self.channels[1]), self.channels[1])
        #
        # self.conv_3_eg = Conv1x1BNReLU(int(2 * self.channels[2]), self.channels[2])
    ...
    def forward(self, x):
        ...
        #     print('bbb',i,x_list[i].shape)

        y_list = self.stage2(x_list)

        # for i in range(len(y_list)):
        #     print('ccc',i ,y_list[i].shape)

        # x_c_1, x_c_2 = y_list
        #
        # y_list[0] = self.conv_1_ac(torch.cat((x_a_1, x_c_1), 1))
        ...
        # for i in range(len(x_list)):
        #     print('ddd',i,x_list[i].shape)

        y_list = self.stage3(x_list)

        # for i in range(len(y_list)):
        #     print('eee',i ,y_list[i].shape)

        # x_e_1, x_e_2, x_e_3 = y_list
        # y_list[0] = self.conv_1_ace(torch.cat((x_a_1, x_c_1, x_e_1), 1))
        # y_list[1] = self.conv_2_ce(torch.cat((x_c_2, x_e_2), 1))
        ...
        # for i in range(len(x_list)):
        #     print('fff',i,x_list[i].shape)

        x = self.stage4(x_list)

        # for i in range(len(x)):
        #     print('ggg',i, x[i].shape)

        # x_g_1, x_g_2, x_g_3, _ = x

        # x[0] = self.conv_1_aceg(torch.cat((x_a_1, x_c_1, x_e_1, x_g_1), 1))
        # x[1] = self.conv_2_ceg(torch.cat((x_c_2, x_e_2, x_g_2), 1))
        # x[2] = self.conv_3_eg(torch.cat((x_e_3, x_g_3), 1))

        # Upsampling
        # x1_h, x1_w = x[1].size(2), x[1].size(3)
        # x1 = F.interpolate(x[1], size=(x1_h, x1_w), mode='bilinear', align_corners=False)
        # x2 = F.interpolate(x[2], size=(x1_h, x1_w), mode='bilinear', align_corners=False)
        # x3 = F.interpolate(x[3], size=(x1_h, x1_w), mode='bilinear', align_corners=False)
...
def hrnet18(args, in_channels, pretrained=True):
    ...
    model = HighResolutionNet(MODEL_CONFIGS['hrnet18_s_v1'], in_channels)

    if pretrained:
        model_path = '../../pretrain_model/hrnet_w18_small_model_v1.pth'
